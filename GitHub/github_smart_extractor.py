...
# Paths
OUTPUT_DIR = "/Users/saeedadeeb/Documents/projects/NLGAD/PyViz/GitHub"
OUTPUT_FILE = "/Users/saeedadeeb/Documents/projects/NLGAD/PyViz/GitHub/github_smart_validated.json"
SEEN_FILE = "/Users/saeedadeeb/Documents/projects/NLGAD/PyViz/GitHub/seen_repos_smart.json"
KEYS_FILE = "/Users/saeedadeeb/Documents/projects/NLGAD/PyViz/GitHub/github_key.json"
    ...
    def search_repos(self, query: str, page: int = 1, per_page: int = 30) -> List[Dict]:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                logging.warning(f"Search failed: {resp.status_code}")
                return []
...
def find_python_files(api: GitHubAPI, owner: str, repo: str, path: str = "", depth: int = 0) -> List[str]:
    ...
    return python_files[:20]  # Limit files per repo


def process_repo(api: GitHubAPI, repo: Dict, seen_hashes: Set[str]) -> List[Dict]:
    ...
    for file_url in python_files[:10]:  # Limit files
        if not file_url:
            continue
...
def main():
    ...
    for query in SEARCH_QUERIES:
        ...
        for page in range(1, 4):  # 3 pages per query
            repos = api.search_repos(query, page=page, per_page=20)
            
            if not repos:
                break
            
            for repo in tqdm(repos, desc=f"'{query}' p{page}", leave=False):
                ...
                # Process repo
                snippets = process_repo(api, repo, seen_hashes)
                
                if snippets:
                    for i, snippet in enumerate(snippets):
                        snippet['file_id'] = f"gh_{repo_full_name.replace('/', '_')}_{len(all_snippets) + i}"
                    all_snippets.extend(snippets)
                    logging.info(f"  ✓ {repo_full_name}: {len(snippets)} validated snippets")
        ...
        logging.info(f"Progress: {len(all_snippets)} validated snippets from {len(seen_repos)} repos")
        
        time.sleep(2)  # Rate limit
    
    logging.info(f"\n{'='*50}")
    logging.info(f"✅ Extracted {len(all_snippets)} VALIDATED snippets from GitHub!")
